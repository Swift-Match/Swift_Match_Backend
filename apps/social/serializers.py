...
class AddMemberSerializer(serializers.Serializer):
    ...
    def validate_user_id(self, user):
        if GroupMembership.objects.filter(
            group=self.context["group"], user=user
        ).exists():
    ...
    def validate(self, data):
        ...
        if request_user == to_user:
            raise serializers.ValidationError(
                "Você não pode enviar um pedido de amizade para você mesmo."
            )
...
class GroupInviteCreateSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if GroupInvite.objects.filter(
            group=group, receiver=receiver, status="PENDING"
        ).exists():
            raise serializers.ValidationError(
                "Já existe um convite pendente para este usuário neste grupo."
            )
