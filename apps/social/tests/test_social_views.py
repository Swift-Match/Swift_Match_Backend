...
def user_a(user_factory):
    return user_factory(username='user_a_view', email='a_view@test.com', password='password123')

@pytest.fixture
def user_b(user_factory):
    return user_factory(username='user_b_view', email='b_view@test.com', password='password123')

@pytest.fixture
def user_c(user_factory):
    return user_factory(username='user_c_view', email='c_view@test.com', password='password123')
...
class TestFriendshipViews:
    ...
    def test_manage_friendship_accept(self, api_client, user_a, user_b):
        ...
        api_client.force_authenticate(user=user_a)
        url = reverse('friendship-manage', kwargs={'pk': friendship.id, 'action': 'accept'})
    ...
    def test_manage_friendship_reject(self, api_client, user_a, user_b):
        ...
        api_client.force_authenticate(user=user_a)
        url = reverse('friendship-manage', kwargs={'pk': friendship.id, 'action': 'reject'})
