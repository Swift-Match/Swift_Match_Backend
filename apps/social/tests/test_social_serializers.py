...
def user_a(user_factory):
    return user_factory(
        username="user_a_test", email="a_test@test.com", password="password123"
    )
...
def user_b(user_factory):
    return user_factory(
        username="user_b_test", email="b_test@test.com", password="password123"
    )
...
def user_c(user_factory):
    return user_factory(
        username="user_c_test", email="c_test@test.com", password="password123"
    )
...
class TestFriendshipRequestSerializer:
    ...
    def test_validation_send_to_self(self, user_a):
        ...
        assert "Você não pode enviar um pedido de amizade para você mesmo." in str(
            excinfo.value
        )
    ...
    def test_validation_request_received(self, user_a, user_b):
        ...
        assert (
            "Este usuário já te enviou um pedido. Aceite-o em vez de enviar outro."
...
class TestGroupInviteCreateSerializer:
    ...
    def test_validation_pending_invite_exists(self, group_owned_by_a, user_c, user_a):
        ...
        assert (
            "Já existe um convite pendente para este usuário neste grupo." in error_msg
            or "unique set" in error_msg
            or "unique" in error_msg
        )

    def test_validation_rejected_invite_allows_new(
        self, group_owned_by_a, user_c, user_a
    ):
