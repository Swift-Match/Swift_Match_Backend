...
def album_fearless(db):
    ...
    return Album.objects.create(
        title="Fearless (Taylor's Version)", release_date=date(2021, 4, 9)
    )
...
class TestTrackModel:
    ...
    def test_track_creation_success(self, album_fearless):
        ...
        track = Track.objects.create(
            album=album_fearless, title="Love Story (Taylor's Version)", track_number=2
        )
    ...
    def test_track_str_representation(self, album_fearless):
        ...
        track = Track.objects.create(
            album=album_fearless, title="Fifteen (Taylor's Version)", track_number=4
        )
    ...
    def test_unique_together_constraint(self, album_fearless):
        ...
        Track.objects.create(
            album=album_fearless, title="You Belong with Me", track_number=5
        )

        with pytest.raises(IntegrityError):
            Track.objects.create(
                album=album_fearless, title="Another Song", track_number=5
            )

    def test_unique_together_allows_same_number_different_album(
        self, album_fearless, album_red
    ):
        """Teste Unitário: Garante que o mesmo track_number pode ser usado em álbuns diferentes."""

        Track.objects.create(
            album=album_fearless, title="Faixa 1 - Fearless", track_number=1
        )
    ...
    def test_track_ordering_meta(self, album_fearless, album_red):
        ...
        Track.objects.create(
            album=album_fearless, title="Faixa 3 Fearless", track_number=3
        )
        Track.objects.create(album=album_red, title="Faixa 1 Red", track_number=1)
        Track.objects.create(
            album=album_fearless, title="Faixa 1 Fearless", track_number=1
