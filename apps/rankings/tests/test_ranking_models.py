...
def test_album_ranking_creation(user_fixture, album_fixture):
    ...
    ranking = AlbumRanking.objects.create(
        user=user_fixture, album=album_fixture, position=1
    )
    assert ranking.user == user_fixture
    assert ranking.position == 1
    assert (
        str(ranking) == f"{user_fixture.username}'s Ranking: {album_fixture.title} (1°)"
    )
...
@pytest.mark.django_db
def test_album_ranking_unique_position_constraint(
    user_fixture, album_fixture, track_fixture
):
    """Garante que um usuário não pode ter dois álbuns na mesma posição."""
    album_b = Album.objects.create(
        title="Album B", release_date="2020-01-01", cover_image_url="http://b.jpg"
    )
    AlbumRanking.objects.create(user=user_fixture, album=album_fixture, position=1)
...
def test_track_ranking_str(user_fixture, track_fixture):
    ...
    ranking = TrackRanking.objects.create(
        user=user_fixture, track=track_fixture, position=10
    )
    assert (
        str(ranking)
        == f"{user_fixture.username}'s Track Ranking: {track_fixture.title} (10°)"
    )
...
def test_group_ranking_str(group_fixture, album_fixture, user_fixture):
    ...
    group_ranking = GroupRanking.objects.create(
        group=group_fixture, album=album_fixture, added_by=user_fixture
    )
...
def test_user_ranking_str(user_fixture, group_fixture, album_fixture):
    ...
    user_ranking = UserRanking.objects.create(
        group_ranking=group_ranking, user=user_fixture
    )
    assert (
        str(user_ranking)
        == f"{user_fixture.username} submeteu {group_ranking.album.title}"
    )


@pytest.mark.django_db
def test_ranked_track_creation(
    user_fixture, group_fixture, album_fixture, track_fixture
):
    ...
    ranked_track = RankedTrack.objects.create(
        user_ranking=user_ranking, track=track_fixture, position=1
    )
