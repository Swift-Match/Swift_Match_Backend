...
def album_fixture(db):
    ...
    return Album.objects.create(
        title="Speak Now (Taylor's Version)", release_date="2023-07-07"
    )
...
def track_fixture(db, album_fixture):
    ...
    return Track.objects.create(album=album_fixture, title="Enchanted", track_number=10)


@pytest.fixture
def group_fixture(db, user_fixture):
    """Cria um grupo de teste."""
    return Group.objects.create(name="Grupo Alpha", owner=user_fixture)
